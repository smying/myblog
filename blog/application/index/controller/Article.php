...
class Article extends Nav
{
    public function index()
    {
    	$id=input('id');//在文章列表页点击进入获取传过来的文章的id值
    	db('article')->where('id', $id)->setInc('click');//click字段自增1，setInc第二个参数为空则默认自增1
        //根据文章id联表查询该文章的栏目名称，获取该文章的信息
    	$arts=\think\Db::name('article')->alias('a')->join('cate c','c.ID = a.cateid','LEFT')->field('a.keywords,a.title,a.content,a.time,a.click,a.id,a.like,a.cateid,c.catename,a.pic')->find($id);
